...
public extension Collection {
    /// SS: The full range of the collection.
...
public extension Collection {
    #if canImport(Dispatch)
    /// SS: Performs `each` closure for each element of collection in parallel.
    ///
    ...
    /// SS: Safe protects the array from out of bounds by use of optional.
    ///
    ///        let arr = [1, 2, 3, 4, 5]
    ///        arr[safe: 1] -> 2
    ///        arr[safe: 10] -> nil
    ...
    /// SS: Returns an array of slices of length "size" from the array. If array can't be split evenly, the final slice will be the remaining elements.
    ///
    ...
    /// SS: Get all indices where condition is met.
    ///
    ...
    /// SS: Calls the given closure with an array of size of the parameter slice.
    ///
...
public extension Collection where Element: BinaryInteger {
    /// SS: Average of all elements in array.
    ///
